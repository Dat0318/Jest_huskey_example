// @ts-ignore
import authorizedRequest from './request/authorizedRequest.ts';
import { OrderModel, OrderList, OrderRequest, CreateOrderRequest } from '../models/order';
...
export function createOrder(data: CreateOrderRequest) {
  return authorizedRequest.post<OrderList, OrderList>(OrderEndPointPrefix, data);
}

export function getDetailOrder(id: string) {
  return authorizedRequest.get<OrderModel, OrderModel>(`${OrderEndPointPrefix}/${id}`);
}

export function cancelOrder(id: string) {
  return authorizedRequest.patch<OrderModel, OrderModel>(`${OrderEndPointPrefix}/${id}/cancel`);
}
