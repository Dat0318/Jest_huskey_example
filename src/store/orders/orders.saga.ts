...
import {
  ...
  // @ts-ignore
} from './orders.action.ts';
// @ts-ignore
import { getMessageFromError, shallowTrim } from '../../helpers/common.util.ts';
// @ts-ignore
...
function* handleCreateOrder(action: ReturnType<typeof createOrderAction.request>) {
  try {
    const response: OrderModel = yield call(createOrder, shallowTrim(action.payload));
...
function* handleCancelOrder(action: ReturnType<typeof cancelOrderAction.request>) {
  try {
    const response: OrderModel = yield call(cancelOrder, action.payload);
    yield put(cancelOrderAction.success(response));
    const { page, pageSize } = yield select((state: StoreState) => state.orders.orders);
...
function* handleGetOrderDetail(action: ReturnType<typeof getOrderDetailAction.request>) {
  try {
...
function* handleCheckoutOrder(action: ReturnType<typeof checkoutOrderAction.request>) {
  try {
